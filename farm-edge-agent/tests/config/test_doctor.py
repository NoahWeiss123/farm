...
def test_missing_api_key_is_critical(fake_device: Path) -> None:
    ...
    findings = check(cfg)
    assert any(
        f.severity is Severity.CRITICAL and f.code is ErrorCode.API_KEY_REJECTED
        for f in findings
    )
...
def test_unexpanded_api_key_is_critical(fake_device: Path) -> None:
    ...
    findings = check(cfg)
    assert any(f.code is ErrorCode.API_KEY_REJECTED for f in findings)
...
def test_xarm_without_arm_ip_is_critical(fake_device: Path) -> None:
    ...
    findings = check(cfg)
    arm_findings = [f for f in findings if f.code is ErrorCode.DRIVER_REQUIRES_ARM_IP]
...
def test_xarm_with_arm_ip_is_ok(fake_device: Path) -> None:
    ...
    assert all(f.code is not ErrorCode.DRIVER_REQUIRES_ARM_IP for f in check(cfg))


def test_camera_device_missing_is_flagged() -> None:
    cfg = _config(camera=CameraConfig(wrist=CameraView(device="/dev/nope-12345")))
    findings = check(cfg)
    cam_findings = [f for f in findings if f.code is ErrorCode.NO_CAMERA]
...
def test_stale_calibration_is_flagged(tmp_path: Path, fake_device: Path) -> None:
    ...
    findings = check(cfg)
    stale = [f for f in findings if f.code is ErrorCode.CALIBRATION_STALE]
    assert len(stale) == 1
...
def test_fresh_calibration_not_flagged(tmp_path: Path, fake_device: Path) -> None:
    ...
    assert all(f.code is not ErrorCode.CALIBRATION_STALE for f in check(cfg))
...
def test_every_finding_has_error_code_and_fix() -> None:
    ...
    for f in findings:
        ...
        assert f.code.value.startswith("FARM-E")
        assert f.fix
